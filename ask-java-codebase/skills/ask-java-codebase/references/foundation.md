# Foundation: Project Identification, Framework Detection, DI, Lombok

Load this when the question is about what stack the project uses, how it's wired (DI), package layout, bootstrap, or whenever you're unsure where to start. This is the foundation for every other reference.

For messaging / HTTP clients / gRPC / GraphQL / OpenAPI, see [integrations.md](./integrations.md). For JPA and the data layer, see [data-layer.md](./data-layer.md).

## Contents

- **1. Project Identification** — build-system indicators, Maven/Gradle analysis, source layout
- **2. Framework Detection (core)** — Spring Boot, Spring WebFlux (reactive), Mutiny/Quarkus, Jakarta EE / Micronaut / Quarkus
- **3. Dependency Injection** — injection styles, scopes, qualifiers, conditional beans
  - **Lombok-Aware Reading** — what each Lombok annotation generates and its analysis impact
...
## 2. Framework Detection (core)

This covers the frameworks that determine the request, threading, and security model. Messaging, HTTP clients, gRPC, GraphQL, and OpenAPI detection live in [integrations.md](./integrations.md); Spring Data JPA detection lives in [data-layer.md](./data-layer.md).
